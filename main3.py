...
async def call_http(request) -> web.StreamResponse:
    print(CHARGE_POINTS)
    data = {}
    params = {}
    params['connectorId'] = 1
    params['idTag'] = "user#10"
    data['params'] = params
    data['method'] = "RemoteStartTransaction"
    data['chargePointId'] = "Baku_Test_Device"
...
async def process_events(address='localhost', port=8080):
    global CHARGE_POINTS
    app = web.Application()
    app["data"] = set()
    app.router.add_route('GET', '/hello', call_http)
    # app.router.add_route('GET', '/handle', handle)
    app.router.add_route('GET', "/ping", pr)
